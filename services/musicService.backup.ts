...
class MusicService {
  ...
  async playTrack(track: MusicTrack, playlist?: MusicTrack[]) {
    try {
      ...
      if (this.sound) {
        try {
          // Remove callback antes de descarregar
  ...
  private onPlaybackStatusUpdate(status: any) {
    ...
    try {
      if (status.isLoaded) {
        ...
        // Atualiza o estado sempre - este é o callback principal e mais confiável
        this.playbackState = {
          ...this.playbackState,
  ...
  async playNext() {
    ...
    if (this.playbackState.shuffleMode) {
      ...
      if (availableIndices.length === 0) {
        nextIndex = this.currentTrackIndex; // Só uma música na playlist
      } else {
    ...
    if (nextTrack) {
      ...
      await this.playTrack(nextTrack, this.currentPlaylist);
    }
  }
  ...
  async playPrevious() {
    ...
    if (this.playbackState.shuffleMode) {
      ...
      if (availableIndices.length === 0) {
        prevIndex = this.currentTrackIndex; // Só uma música na playlist
      } else {
    ...
    if (prevTrack) {
      ...
      await this.playTrack(prevTrack, this.currentPlaylist);
    }
  }
  ...
  private async handleTrackFinished() {
    switch (this.playbackState.repeatMode) {
      case 'one':
        ...
        break;
      case 'all':
        // Ir para próxima faixa (ou primeira se for a última)
        if (this.currentPlaylist.length > 1) {
          await this.playNext();
        } else if (this.currentTrack && this.sound) {
          // Se só tem uma música, repete ela
          await this.sound.replayAsync();
        }
        break;
      case 'off':
      default:
        // Tocar próxima faixa se disponível, senão para
